...
## Step 4: Run Codex / Kimi Code inside a sandbox
...
loopbox harness describe kimi            # notes + launch examples for one harness
...
loopbox harness run $SID kimi  -- -p "summarise this repo"   # Kimi Code headless mode
```
...
| Kimi Code | Paste the "Connect this repo to LoopX" prompt generated by `loopx new-project-prompt`; keep long-running loops alive with goal mode (`/goal`) |
